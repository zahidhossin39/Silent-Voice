...
mod tests {
    ...
    // Live smoke test against the real downloaded model. Ignored by default;
    // run with: cargo test --no-default-features coedit::tests::live_correct -- --ignored --nocapture
    #[test]
    #[ignore]
    fn live_correct() {
        let input = "I was fighting with him in future.";
        let out = correct(input);
        eprintln!("loaded={}  in=\"{}\"  out=\"{}\"", is_loaded(), input, out);
        assert!(is_loaded(), "model did not load — init_coedit returned None");
        assert_ne!(out, input, "model loaded but produced no correction");
    }
