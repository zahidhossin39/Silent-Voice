# Silent Voice — Full Project Handbook

This is the deep reference for the project: architecture, subsystem histories,
and the full rationale behind every "must never change" rule. `CLAUDE.md` at
...
33 GB `target/` cleanup (build caches deleted — first build after checkout
is cold).
...
- `shadow(false)` — shadow off; corners are rounded by round_corners() in
  overlay.rs: DWM `DWMWA_WINDOW_CORNER_PREFERENCE` on Windows 11, with a
  `SetWindowRgn(CreateRoundRectRgn)` fallback on Windows 10 (where that DWM
  attribute doesn't exist and the opaque pill showed square corners). The
  Win10 region is size-specific, so `apply_size()` re-applies it after every
  resize (right-click menu). The DWM attribute is (re)attempted on every call
  — cheap, idempotent, and correct even if the window is ever recreated.
...
- **Inline proofreading — system-wide squiggles (Grammarly Phase 2, v0.1.4,
  ...
  - **WebView2 apps — WhatsApp Desktop etc. (works, do not regress):**
    ...
    `inline_check.rs`): set `SPI_SETSCREENREADER`(TRUE) AND register a COM
    `IUIAutomation::AddFocusChangedEventHandler` (keep the
    ...
    build a11y trees (CPU/mem cost on weak machines). Since post-0.1.6 it is
    engaged LAZILY in the watcher loop only while the "Inline proofreading"
    toggle is ON, and disengaged (handler removed + flag reset) when the
    toggle turns off — an app that never enables the feature costs the system
    nothing. It is also RESET on app Exit (`reset_screen_reader()` in the
    RunEvent::Exit handler) so the system doesn't stay in accessibility mode.
    Verified live: 3 squiggles render over WhatsApp's compose box for
    misspellings.
...
- **Doubled-word transcription artifact fix (v0.1.6):**
...
- **Hyphenated-word duplication fix (post-0.1.6):** users still saw doubles
  like "post-writing post-writing" AFTER the collapse fix — the collapse was
  fine; `format_numbers()` was CREATING the doubles downstream (it runs after
  collapse, so nothing cleaned them up). Its flattening step split every
  hyphenated token into hyphen parts for number parsing, and the non-number
  fallback emitted the whole original token once per part. Fixed: a
  hyphenated token only expands when EVERY part is a number word
  ("twenty-five" → 25 still works); anything else stays one word. Regression
  test `hyphenated_words_not_duplicated`.
